...
@section('content')
    ...
    <form action="{{ URL::current() }}" method="get" class="row m-2 g-3 align-itmes-end m-2 mt-3">
        <div class="col-md-3">
            <x-form.input name="name" label="اسم الموظف" placeholder="بحث عن موظف..."
                value="{{ request()->query('name') }}" />
        </div>
        <div class="col-md-3">
            <x-form.select name="department_id" class="form-control" label="القسم" :options="$departments->prepend('كل الأقسام', '')"
            :value="request()->query('department_id')" 
            :selected="request()->query('department_id')" />
        </div>
        <div class="col-md-3">
            <x-form.select name="status" class="form-control" label="الحالة" :options="$statusLabels"
                        ...
                        @foreach ($employees as $employee)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <form action="{{ route('dashboard.hr.employees.destroy', $employee->id) }}"
...
@push('scripts')
    ...
    <script>
        document.getElementById('resetBtn').addEventListener('click', function() {
            ...
            window.location.href = "{{ route('dashboard.hr.employees.index') }}";
        });
    </script>
@endpush
